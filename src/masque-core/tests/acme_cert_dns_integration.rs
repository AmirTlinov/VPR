...
use anyhow::Result;
use masque_core::acme_client::{AcmeClient, AcmeClientConfig, ChallengeType};
use masque_core::cert_manager::{CertificateInfo, CertificateManager, CertificateManagerConfig};
...
async fn test_certificate_manager_initialization() -> Result<()> {
    ...
    assert!(result.is_err(), "Manager creation should fail with invalid ACME URL");
...
async fn test_acme_dns01_challenge_calculation() -> Result<()> {
    use masque_core::acme_client::{AcmeAccountKey, AcmeClient, Challenge};
    ...
    assert_eq!(account_key.public_key_jwk().get("kty").unwrap().as_str(), Some("OKP"));
    assert_eq!(account_key.public_key_jwk().get("crv").unwrap().as_str(), Some("Ed25519"));
...
async fn test_dns_updater_factory_http_api() -> Result<()> {
    let mut credentials = HashMap::new();
    credentials.insert("api_url".to_string(), "https://api.example.com/dns".to_string());
...
async fn test_dns_updater_factory_route53() -> Result<()> {
    ...
    credentials.insert("secret_access_key".to_string(), "test-secret-key".to_string());
...
async fn test_dns_updater_verify_txt_record_real_dns() -> Result<()> {
    use masque_core::dns_updater::{DnsUpdater, CloudflareUpdater};
...
async fn test_dns_updater_verify_txt_record_format() -> Result<()> {
    ...
    match lookup_result {
        Ok(lookup) => {
            ...
            assert!(!records.is_empty(), "Should have TXT records for example.com");
...
async fn test_certificate_manager_with_dns_updater() -> Result<()> {
    ...
    dns_credentials.insert("api_url".to_string(), "https://api.example.com/dns".to_string());
