...
pub fn draw(frame: &mut Frame<'_>, globe: &GlobeRenderer, area: Rect, angle: f32, stats: UiStats) {
    ...
    let ascii = globe.render_frame(globe_area.width as usize, globe_area.height as usize, angle, stats.tick);
    
    // Apply a "CRT" scanline effect or just direct rendering
    frame.render_widget(
        ascii
            .to_paragraph()
            .block(Block::default().borders(Borders::NONE).style(Style::default().fg(Color::Cyan))),
        globe_area,
    );
...
fn render_header(frame: &mut Frame<'_>, area: Rect) {
    ...
    let line = Line::from(vec![
        Span::styled(" VPR_OS ", Style::default().fg(Color::Black).bg(Color::Cyan).add_modifier(Modifier::BOLD)),
        Span::styled(" :: ", Style::default().fg(Color::DarkGray)),
        Span::styled("SECURE_CHANNEL_ESTABLISHED", Style::default().fg(Color::Green)),
        Span::styled(" :: ", Style::default().fg(Color::DarkGray)),
        Span::styled(format!("T-{}", timestamp % 10000), Style::default().fg(Color::Red)),
        Span::raw(" "),
        Span::styled(" [ENCRYPTED] ", Style::default().fg(Color::LightBlue).add_modifier(Modifier::RAPID_BLINK)),
    ]);

    frame.render_widget(Paragraph::new(line).alignment(ratatui::layout::Alignment::Left), area);
}
...
fn render_info_panel(frame: &mut Frame<'_>, area: Rect, stats: &UiStats) {
    ...
    // 1. Network Status
    let net_status = format!("UPLINK: {} Mbps\nLATENCY: {} ms", stats.throughput_mbps, stats.latency_ms);
    frame.render_widget(
        Paragraph::new(net_status)
            .block(Block::default().borders(Borders::ALL).border_style(Style::default().fg(Color::LightGreen)).title("NET_STAT")),
        blocks[0],
    );
    ...
    frame.render_widget(
        Paragraph::new(log_lines)
            .block(Block::default().borders(Borders::LEFT).title("MEM_DUMP")),
        blocks[1],
    );
    ...
    let proc_text: Vec<Line> = processes.iter().enumerate().map(|(i, p)| {
...
fn render_footer(frame: &mut Frame<'_>, area: Rect, stats: &UiStats) {
    ...
    let line = Line::from(vec![
        ...
        Span::styled(" [Q: ABORT] ", Style::default().fg(Color::Red).add_modifier(Modifier::BOLD)),
    ]);
